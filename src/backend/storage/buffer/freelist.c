...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	*lock_held = true;
	...
	int ord;
	ord=NBuffers;
	for(ind = 0; ind < NBuffers; ind++){
		if( (BufferDescriptors[ind].refcount == 0) && BufferDescriptors[ind].order < ord ) {
			buf = &BufferDescriptors[ind];
			ord = buf->order;
		}
	}
...
/*
	...
	This finds the maximum order value of a buffer
...
unsigned maxOrder(){
	...
	for(i=0; i<NBuffers; i++)
		if ( BufferDescriptors[i].order > max )
			max = BufferDescriptors[i].order;
...
void clean(){
	...
	int i;
	for(i=0; i<NBuffers ; i++)
		if(BufferDescriptors[i].order > 0)
			notZeros++;

	for(i=1; i <= notZeros ; i++){
		int j;
		int min=NBuffers+10;
		for(j=0;j<NBuffers; j++)
			if ( (BufferDescriptors[j].order >=i) && (BufferDescriptors[j].order < min) ){
				min = BufferDescriptors[j].order;
				buf = &BufferDescriptors[j];
			}
		buf->order=i;
	}
}
